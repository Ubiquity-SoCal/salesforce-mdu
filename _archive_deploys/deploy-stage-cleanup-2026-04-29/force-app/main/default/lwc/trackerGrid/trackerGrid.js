...
export default class TrackerGrid extends NavigationMixin(LightningElement) {
    ...
    handleViewChange(event) {
        ...
        // Reset only column-level filter (stage-specific). Preserve Owner/RE/Date/Campaign so users
        // can flip stages while keeping cross-stage scoping (e.g. "all my ROE Secured" then switch
        // to "all my Under Contract" without re-picking RE Assigned).
        this.filterField = '';
        this.filterValue = '';
        this.loadViewData();
    }
    ...
    async loadViewData() {
        ...
        try {
            ...
            // If user has Owner/RE/Date/Campaign filters carried over from a prior view, apply them now.
            if (this.selectedOwner || this.selectedREAssigned || this.selectedDateRange || this.selectedCampaign) {
    ...
    buildOwnerOptions() {
        ...
        const mduTeam = ['Bill Holick', 'Brett Spivey', 'Chuck McNeely', 'Melissa Baker', 'Taylor Mauney',
                         ...
                         'Rosemarie Shortino', 'Justin Barry'];
