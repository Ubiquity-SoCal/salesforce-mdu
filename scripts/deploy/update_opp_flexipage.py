"""
Update Opportunity FlexiPage — CANONICAL SOURCE OF TRUTH for the Opp record page.
Any field additions/removals should go through this script's sections definition,
NOT through the Lightning page editor (UI changes get overwritten on next deploy).

Deploys via Metadata API.

Current fields (as of 2026-03-31):
  Opportunity Info: Name, AccountId, Portfolio, Management_Company, Contact, Agreement_Name
                    OwnerId, Market_Sales_Lead, RE_Assigned, CloseDate, StageName, Probability, Amount
  Property Details: Property_Unit, Units, Property_Type, Property_Category, Build_Type
                    Property_Address, Property_City, Property_State, Property_Zip
  ISP Information:  Prospective_ISP | Confirmed_ISP
  Additional Info:  NextStep, Description | LeadSource
  Integration:      SiteTracker_Project_ID, SiteTracker_URL | IronClad_URL
  Migration:        Monday_Item_ID
  System:           CreatedById | LastModifiedById
"""
...
# =============================================================================
# FIELD SECTIONS — Edit here to add/remove/reorder fields on the Opp page
# Format: (Section Label, section_id,
#           [left_column_fields], [left_behaviors],
#           [right_column_fields], [right_behaviors])
# Behaviors: "none" (editable), "readonly", "required"
# =============================================================================
sections = [
    ("Opportunity Information", "s1",
     ["Name", "AccountId", "Portfolio__c", "Management_Company__c", "Contact__c", "Agreement_Name__c"],
...
# Related lists in left sidebar
left_sidebar_lists = "\n".join([
    related_list_xml("Opportunity_Contacts__r", "force_relatedListSingleContainer_oppContacts", "true"),
    related_list_xml("Agreements__r", "force_relatedListSingleContainer", "true"),
    related_list_xml("SiteTracker_Projects__r", "force_relatedListSingleContainer_stProjects", "false"),
    related_list_xml("AttachedContentNotes", "force_relatedListSingleContainer2", "true"),
])
...
print("Deploying updated Opportunity FlexiPage...")
resp = requests.post(
    deploy_url,
