...
VF_PAGE_FILE = "InsideSalesDashboard_new.page"


def load_markup():
    """Load VF markup from the .page file."""
    import os
    page_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), VF_PAGE_FILE)
    with open(page_path, "r", encoding="utf-8") as f:
        return f.read()


# Keep inline markup as fallback — but load_markup() is used by default
...
<body>
    ...
    var activeRecordType = 'All';

    function loadDashboard(recordType) {
    activeRecordType = recordType || 'All';
    var rtFilter = '';
    if (activeRecordType === 'MDU') rtFilter = " AND RecordType.Name = 'MDU'";
    else if (activeRecordType === 'Business') rtFilter = " AND RecordType.Name = 'Business'";
    ...
    Promise.all([
        ...
        // 7: Under Contract by month 2026
        ...
        var underContract = results[1].records[0];
        ...
        var myUnderContract = results[10].records[0];
        ...
        // Slicer buttons
        html += '<div style="display:flex;gap:4px;margin-right:12px;">';
        ['All','MDU','Business'].forEach(function(rt) {
            var isActive = activeRecordType === rt;
            html += '<button onclick="loadDashboard(\'' + rt + '\')" style="padding:4px 14px;font-size:0.75rem;font-weight:600;border-radius:4px;cursor:pointer;border:1px solid ' + (isActive ? '#0176d3' : '#c9c7c5') + ';background:' + (isActive ? '#0176d3' : '#fff') + ';color:' + (isActive ? '#fff' : '#3e3e3c') + ';">' + rt + '</button>';
        });
        html += '</div>';
        ...
        html += '<div class="metric-card"><div class="metric-label">Under Contract</div><div class="metric-value">' + underContract.cnt + '</div><div class="metric-sub">' + fmt(underContract.total) + '</div></div>';
        ...
        html += '<div class="metric-card"><div class="metric-label">My Under Contract</div><div class="metric-value">' + myUnderContract.cnt + '</div><div class="metric-sub">' + fmt(myUnderContract.total) + '</div></div>';
...
def main():
    ...
    markup = load_markup()
    print(f"Loaded markup from {VF_PAGE_FILE} ({len(markup)} chars)")
