...
root = ET.fromstring(flexipage_content)
ns = {'md': 'http://soap.sforce.com/2006/04/metadata'}

for region in root.findall('.//md:flexiPageRegions', ns):
    region_name = region.find('md:name', ns)
    region_type = region.find('md:type', ns)
    rname = region_name.text if region_name is not None else '?'
    rtype = region_type.text if region_type is not None else '?'

    components = []
    for item in region.findall('.//md:componentInstance', ns):
        comp_name = item.find('md:componentName', ns)
        identifier = item.find('md:identifier', ns)
        # Check for relatedListApiName
        for prop in item.findall('md:componentInstanceProperties', ns):
            pname = prop.find('md:name', ns)
            pval = prop.find('md:value', ns)
            if pname is not None and pname.text == 'relatedListApiName' and pval is not None:
                components.append(f"relatedList:{pval.text}")
                break
        else:
            if comp_name is not None:
                components.append(comp_name.text)

    comp_str = ', '.join(components) if components else '(empty)'
    print(f"    {rname} ({rtype}): {comp_str}")
...
# Step 2: Add Notes + Opportunity Contacts
print("\nStep 2: Injecting Notes + Opportunity Contacts...")
...
if merge_pos > 0:
    ...
    print("  Injected before merge card")
    injected = True

# Strategy 2: Before leftsidebar/sidebar name tag
if not injected:
    for sidebar_name in ['leftsidebar', 'sidebar', 'Sidebar']:
        marker = f'<name>{sidebar_name}</name>'
        pos = flexipage_content.find(marker)
        if pos > 0:
            last_item = flexipage_content.rfind('</itemInstances>', 0, pos)
            if last_item > 0:
                insert_pos = last_item + len('</itemInstances>')
                flexipage_content = flexipage_content[:insert_pos] + '\n' + notes_item + opp_contacts_item + flexipage_content[insert_pos:]
                print(f"  Injected before {sidebar_name} closing")
                injected = True
                break

# Strategy 3: Before the first </flexiPageRegions> that contains related lists
if not injected:
    # Just add before the first region closing
    first_region_end = flexipage_content.find('</flexiPageRegions>')
    if first_region_end > 0:
        flexipage_content = flexipage_content[:first_region_end] + notes_item + opp_contacts_item + flexipage_content[first_region_end:]
        print("  Injected in first region")
        injected = True
    ...
    for j in range(15):
        ...
        if status in ('Succeeded', 'Failed', 'Canceled'):
            if status == 'Failed':
                ...
                print("\n  Contact Record Page updated!")
                print("  Added: Notes + Opportunity Contacts")
            break
else:
